...
<?php
	...
	if (isset($_POST["log_start_1"])){
		...
		$cmd = cmd_docker(1)." '".cmd_rtl_sdr($config, 1)." | ".cmd_liquidsdr($config, 1).cmd_sql($config, 1, $run_id)." >> ". $file_path." 2>&1'";
		start_docker_echo($cmd,'tab_logger_range','Started Receiver 2');
	}
