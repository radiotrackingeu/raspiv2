...
if [ "$1" = 'mysqld' ]; then
    ...
    mysql_install_db --datadir="$DATADIR"
		...
		cat >> "$tempSqlFile" <<-EOSQL
				...
				CREATE TABLE rteu.signals (\`id\` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY, \`timestamp\` CHAR(30), \`samples\` BIGINT, \`duration\` FLOAT, \`signal_freq\` FLOAT, \`signal_bw\` FLOAT, \`max_signal\` FLOAT, \`run\` INT UNSIGNED NOT NULL, FOREIGN KEY(\`run\`) REFERENCES rteu.runs(\`id\`) ON DELETE CASCADE);
